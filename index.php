...
<div class="row text-light" id="game" v-if="war === false">
    ...
    <div class="col-8">
        <div class="row">
            <div class="col-3">
                <power :fire-percent="firePercent" :earth-percent="earthPercent" :water-percent="waterPercent" :air-percent="airPercent"></power>
            </div>
            <div class="col-9" v-if="(timeTilWar !== 0)">
                <h4>Next war info</h4>
                <h5>Time until next war: {{timeTilWar}}{{ (timeTilWar !== 1) ? " days" : " day"}}</h5>
                <h5>Location of war: <span :class="translation(locationOfWar, '')">The {{translation(locationOfWar, "dN")}}</span></h5>
            </div>
        </div>
        <div class="mainMap">
        ...
        <div class="col-8 my-2">
            <div class="row">
                ...
                <div class="warInfo col-9">
                    ...
                    <h5>Scale of the war: {{ (battleScale === 0) ? "Minor battle" : (battleScale === 1) ? "Moderate battle": "Major battle" }}</h5>
                    <h5>Armies Involved: <span :class="army.name" v-for="(army,index) in armiesInvolved">{{translation(army.name, "dN")}}{{(index+1 !== armiesInvolved.length)? ", ": ""}}</span></h5>
            ...
            <div class="modal-content bg-dark text-light">
                ...
                <div class="modal-body">
                    ...
                    <div v-else-if="startModalTab === 1">
                        ...
                        <button class="btn btn-primary btn-large btn-block" v-if="playerNation === location" @click="warSolution= 'fight';fightMethod();">Fight?</button>
                        <button class="btn btn-primary btn-large btn-block" @click="warSolution= 'diplomacy';diplomacyMethod();">Diplomacy? (Talk it out)</button>
                        <button class="btn btn-primary btn-large btn-block" @click="warSolution= 'nothing';endWar();">Do nothing</button>
                    </div>
                    <div v-else-if="startModalTab === 2">
                        <h3>Who would you like to side with</h3>
                        <button class="btn btn-primary btn-large btn-block" @click="(warSolution === 'fight') ? fight.chosenSide = armiesInvolved[0].name : diplomacy.chosenSide = armiesInvolved[0].name; (warSolution === 'fight') ? fightMethod() : diplomacyMethod();">The {{translation(armiesInvolved[0].name, "dN")}}</button>
                        <button class="btn btn-primary btn-large btn-block" @click="(warSolution === 'fight') ? fight.chosenSide = armiesInvolved[1].name : diplomacy.chosenSide = armiesInvolved[1].name; (warSolution === 'fight') ? fightMethod() : diplomacyMethod();">The {{translation(armiesInvolved[1].name, "dN")}}</button>
                    </div>
                </div>
        ...
        <div class="modal-content bg-dark text-light">
            ...
            <div class="modal-body">
                <div class="stats tab"v-if="tab === 0">
                    <h3>Stats</h3>
                    <h5 :class="currentPosition">You are in the {{(currentPosition !== 'ocean')? translation(currentPosition, "dN") : "Ocean"}}</h5>
                    <h4>Skill</h4>
                    <h5 class="fireSkill fire">You are a{{(skill.fire.level === 1 || skill.fire.level === 2) ? "n": ""}} {{skill.levels[skill.fire.level]}} firebender and {{format("percent",skill.fire.percent)}} through your rank</h5>
                    <h5 class="waterSkill water">You are a{{(skill.water.level === 1 || skill.water.level === 2) ? "n": ""}} {{skill.levels[skill.water.level]}} waterbender and {{format("percent",skill.water.percent)}} through your rank</h5>
                    <h5 class="airSkill air">You are a{{(skill.air.level === 1 || skill.air.level === 2) ? "n": ""}} {{skill.levels[skill.air.level]}} airbender and {{format("percent",skill.air.percent)}} through your rank</h5>
                    <h5 class="earthSkill earth">You are a{{(skill.earth.level === 1 || skill.earth.level === 2) ? "n": ""}} {{skill.levels[skill.earth.level]}} earthbender and {{format("percent",skill.earth.percent)}} through your rank</h5>
                </div>
                    ...
                    <div class="container-fluid">
                        <div class="action-row row" v-for="i in Math.ceil(actions.length/2)">
                            <div class="action col-6 my-1 mx-0" v-for="action in actions.slice((i - 1) * 2, i * 2)">
                                <button :class="(action.cost > timeTilWar) ? 'disabled': ''" :disabled="(action.cost > timeTilWar) ? true: false" class="btn btn-dark btn-block btn-large" @click="if(ss(action.cost,action.s)){action.run();}"><span v-html="action.name()"></span><br>Takes you {{action.cost}} days to complete</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab" v-else-if="tab === 'move'">
                    <button class="btn btn-primary btn-lg btn-block" v-if="currentPosition !== 'fire'" @click="actions[1].nation = 'fire'; tab = 1; actions[1].run2();">Fire Nation</button>
                    <button class="btn btn-primary btn-lg btn-block" v-if="currentPosition !== 'water'" @click="actions[1].nation = 'water'; tab = 1; actions[1].run2();">Water Tribe</button>
                    <button class="btn btn-primary btn-lg btn-block" v-if="currentPosition !== 'air'" @click="actions[1].nation = 'air'; tab = 1; actions[1].run2();">Air Nomads</button>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content bg-dark text-light">
            ...
            <div class="modal-body text-center">
                <div v-if="stage === 1">
                    <p>You are a young avatar who has not completed your training in the four elements, Fire, Water, Air, and Earth</p>
                    <a target="_blank" href="https://en.wikipedia.org/wiki/Avatar:_The_Last_Airbender#Series_overview">Don't have any clue what avatar is? Click Here!</a><br>
                    <div class="modal-footer">
                ...
                <div v-else-if="stage === 3">
                    ...
                    <div class="row">
                        <button class="btn btn-primary btn-lg btn-block" @click="stage++; nation = 'fire'; startRand();">Fire Nation</button>
                        <button class="btn btn-primary btn-lg btn-block" @click="stage++; nation = 'water'; startRand();">Water Tribe</button>
                        <button class="btn btn-primary btn-lg btn-block" @click="stage++; nation = 'air'; startRand();">Air Nomads</button>
                        <button class="btn btn-primary btn-lg btn-block" @click="stage++; nation = 'earth'; startRand();">Earth Kingdom</button>
                    </div>
                </div>
